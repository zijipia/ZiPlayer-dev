...
class DummyPlugin extends BasePlugin {
  ...
  canHandle(q){ return q?.startsWith('dummy:'); }
  async search(query, requestedBy){ return { tracks: [{ id:'1', title: query, url:'https://dummy/1', duration:1, requestedBy, source: this.name }] }; }
  async getStream(track){ const { Readable } = require('node:stream'); return { stream: Readable.from([Buffer.from('x')]), type: 'arbitrary' }; }
}
...
test('PlayerManager extension activation by name and ctor', () => {
  ...
  class DummyExt {
    ...
    version = '0.0.0';
    player = null;
    active(ctx){ activated++; return true; }
  }
